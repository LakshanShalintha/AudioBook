...
function BiographiesMemories() {
    const navigate = useNavigate();
    const [pdfFiles, setPdfFiles] = useState([]);
    const [loading, setLoading] = useState(true); // Loading state
    const [favorites, setFavorites] = useState([]); // Favorite state
    ...
    const fetchPdfs = async () => {
        ...
        const folderRef = ref(
            storage,
            "gs://audirab-44b07.appspot.com/Categories/Biographies & Memories"
        );
    ...
    const toggleFavorite = (pdf) => {
        setFavorites((prevFavorites) =>
            prevFavorites.includes(pdf.name)
                ? prevFavorites.filter((item) => item !== pdf.name) // Remove favorite
                : [...prevFavorites, pdf.name] // Add favorite
        );
    };
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-white flex flex-col">
            ...
            {/* Page title */}
            <h1 className="text-center text-4xl mt-28 font-bold ">
            ...
            <div className="flex-grow flex justify-center items-center">
                <div className="max-w-5xl w-full">
                    {loading ? (
                        ...
                        <div
                            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-2 gap-y-4 justify-center -mt-20 mb-40 px-4">
...
export default BiographiesMemories;
